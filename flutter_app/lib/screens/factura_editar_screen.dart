...
class FacturaEditarScreen extends StatefulWidget {
  const FacturaEditarScreen({
    super.key,
  ...
  @override
  State<FacturaEditarScreen> createState() => _FacturaEditarScreenState();
}

class _FacturaEditarScreenState extends State<FacturaEditarScreen> {
  ...
  void initState() {
    ...
    _tipoFactura = f.tipoFactura;
    _clienteId = f.clienteId;
    _fechaEmision = f.fechaEmision;
    _fechaVencimiento = f.fechaVencimiento;
    _estado = f.estado;
    _monedaCodigo = f.monedaCodigo;
    _notasController.text = f.notas ?? '';
    _formaPagoController.text = f.formaPago ?? '';
    _cargarDatos();
  }
  ...
  Future<void> _guardar() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _error = 'No se pudo actualizar la factura';
        _guardando = false;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Editar ${widget.factura.serie}-${widget.factura.numeroFactura}'),
      ),
      body: _cargando
          ...
          : ListView(
              ...
              children: [
                DropdownButtonFormField<String>(
                  ...
                  onChanged: _guardando ? null : (v) => setState(() => _tipoFactura = v ?? 'normal'),
                ),
