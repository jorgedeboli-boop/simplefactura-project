    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text('${_factura.serie}-${_factura.numeroFactura}', maxLines: 1, overflow: TextOverflow.ellipsis),
          actions: [
            Padding(
                ...
                : ListView(
                    ...
                    children: [
                      ...
                      if (_factura.clienteNombre != null)
                        _CampoDetalle(etiqueta: 'Cliente', valor: _factura.clienteNombre!),
                      _CampoDetalle(etiqueta: 'Fecha emisión', valor: _formatoFecha.format(_factura.fechaEmision)),
                      _CampoDetalle(
                      ...
                      if (_factura.formaPago?.isNotEmpty == true)
                        _CampoDetalle(etiqueta: 'Forma de pago', valor: _factura.formaPago!),
                      _CampoDetalle(etiqueta: 'Moneda', valor: _factura.monedaCodigo),
                      if (_factura.notas?.isNotEmpty == true)
                        _CampoDetalle(etiqueta: 'Notas', valor: _factura.notas!),
                      const SizedBox(height: 16),
                      ...
                      if (_lineas.isEmpty)
                        ...
                        ..._lineas.map((l) => _LineaLectura(linea: l, moneda: _factura.monedaCodigo)),
                      const SizedBox(height: 24),
                      Container(
                        ...
                        child: Column(
                          children: [
                            _FilaTotal(etiqueta: 'Subtotal', valor: _moneda(_factura.subtotal)),
                            const SizedBox(height: 8),
                            _FilaTotal(etiqueta: 'IVA', valor: _moneda(_factura.totalIva)),
                            const Divider(height: 24),
                            _FilaTotal(etiqueta: 'Total', valor: _moneda(_factura.total), destacado: true),
                          ],
                        ),
                      ),
                    ],
